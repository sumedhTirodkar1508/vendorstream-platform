...
import { STORE_EXCEL_REQUIRED_HEADERS } from "@vendorstream/contracts";
...
function buildPrevalidationCsvRows(summary: Record<string, unknown>) {
  return [
    ...
    [
      ...
      "Use the required store workbook headers exactly, then upload the corrected workbook again.",
    ],
  ];
}
...
function buildRowValidationCsvRows(
  ...
  const header = ["Source Row Number", "Errors", ...STORE_EXCEL_REQUIRED_HEADERS];
  ...
  if (rows.length === 0) {
    return [
      ...
      [
        ...
        ...STORE_EXCEL_REQUIRED_HEADERS.map(() => ""),
      ],
    ];
  }
  ...
  return [
    ...
    ...rows.map((row) => {
      ...
      return [
        ...
        ...STORE_EXCEL_REQUIRED_HEADERS.map((column) => rawData[column] ?? ""),
      ];
    }),
  ];
}
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (!batch || batch.sourceType !== "STORE") {
    return NextResponse.json(
      {
        error: "Store import batch was not found.",
      },
      { status: 404 },
    );
  }
  ...
  const hasAccess =
    isAdmin ||
    batch.cycle.storeLocation.storeOrganization.memberships.length > 0;
  ...
  const csvRows =
    batch.status === "PREVALIDATION_FAILED"
      ...
      : buildRowValidationCsvRows(batch.rawStoreRows);
  ...
  return new NextResponse(csv, {
    ...
    headers: {
      ...
      "content-disposition": `attachment; filename="store-upload-${batch.id}-validation-errors.csv"`,
    },
  });
}
